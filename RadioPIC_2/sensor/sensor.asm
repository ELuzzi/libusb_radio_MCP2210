...
;sensor.c,212 :: 		address_high = (((short int)(address >> 3)) & 0b01111111) | 0x80;  // calculating addressing mode
	...
	MOVWF       R1 
	MOVF        R2, 0 
L__write_ZIGBEE_long204:
	BZ          L__write_ZIGBEE_long205
	RRCF        R1, 1 
	RRCF        R0, 1 
	BCF         R1, 7 
	BTFSC       R1, 6 
...
;sensor.c,228 :: 		address_high = ((short int)(address >> 3) & 0b01111111) | 0x80;  //calculating addressing mode
	...
	MOVWF       R1 
	MOVF        R2, 0 
L__read_ZIGBEE_long208:
	BZ          L__read_ZIGBEE_long209
	RRCF        R1, 1 
	RRCF        R0, 1 
	BCF         R1, 7 
	BTFSC       R1, 6 
...
L_write_TX_normal_FIFO5:
	...
	SUBWF       R0, 0 
	BTFSS       STATUS+0, 2 
	GOTO        L__write_TX_normal_FIFO215
	MOVLW       18
	SUBWF       write_TX_normal_FIFO_i_L0+0, 0 
L__write_TX_normal_FIFO215:
	BTFSC       STATUS+0, 0 
...
;sensor.c,346 :: 		set_not_ACK();
	CALL        _set_not_ACK+0, 0
;sensor.c,347 :: 		set_not_encrypt();
...
;sensor.c,1102 :: 		Initialize();                      // Initialize MCU and Bee click board
	CALL        _Initialize+0, 0
;sensor.c,1104 :: 		while(1) {
L_main184:
;sensor.c,1106 :: 		delay_ms(100);
	MOVLW       2
	MOVWF       R11, 0
	MOVLW       69
	MOVWF       R12, 0
...
L_main186:
	...
	NOP
	NOP
;sensor.c,1108 :: 		DATA_TX[0]=dig1;
	MOVF        _dig1+0, 0 
	MOVWF       _DATA_TX+0 
;sensor.c,1109 :: 		DATA_TX[1]=dig2;
	MOVF        _dig2+0, 0 
	MOVWF       _DATA_TX+1 
;sensor.c,1110 :: 		DATA_TX[2]=dig3;
	MOVF        _dig3+0, 0 
	MOVWF       _DATA_TX+2 
;sensor.c,1111 :: 		DATA_TX[3]=degrees;
	MOVF        _degrees+0, 0 
	MOVWF       _DATA_TX+3 
;sensor.c,1112 :: 		DATA_TX[4]=battery;
	MOVF        _battery+0, 0 
	MOVWF       _DATA_TX+4 
;sensor.c,1113 :: 		write_TX_normal_FIFO();
