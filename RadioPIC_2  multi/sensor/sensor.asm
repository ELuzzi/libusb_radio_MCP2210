...
;sensor.c,795 :: 		set_channel(ch);     // Channel 11
	MOVF        _ch+0, 0 
	MOVWF       FARG_set_channel_channel_number+0 
...
;sensor.c,1095 :: 		LATD  = 0;             // Clear LATD register
	CLRF        LATD+0 
;sensor.c,1097 :: 		Lcd_Init();                        // Initialize LCD
	CALL        _Lcd_Init+0, 0
;sensor.c,1098 :: 		Lcd_Cmd(_LCD_CLEAR);               // Clear display
	MOVLW       1
	MOVWF       FARG_Lcd_Cmd_out_char+0 
	CALL        _Lcd_Cmd+0, 0
;sensor.c,1099 :: 		Lcd_Cmd(_LCD_CURSOR_OFF);          // Cursor off
	MOVLW       12
	MOVWF       FARG_Lcd_Cmd_out_char+0 
	CALL        _Lcd_Cmd+0, 0
;sensor.c,1102 :: 		SPI1_Init_AdvancEd(_SPI_MASTER_OSC_DIV4, _SPI_DATA_SAMPLE_MIDDLE, _SPI_CLK_IDLE_LOW, _SPI_LOW_2_HIGH);
...
;sensor.c,1103 :: 		pin_reset();                              // Activate reset from pin
	CALL        _pin_reset+0, 0
;sensor.c,1104 :: 		software_reset();                         // Activate software reset
	CALL        _software_reset+0, 0
;sensor.c,1105 :: 		RF_reset();                               // RF reset
	CALL        _RF_reset+0, 0
;sensor.c,1106 :: 		set_WAKE_from_pin();                      // Set wake from pin
	CALL        _set_wake_from_pin+0, 0
;sensor.c,1108 :: 		set_long_address(ADDRESS_long_2);         // Set long address
	MOVLW       _ADDRESS_long_2+0
	MOVWF       FARG_set_long_address_address+0 
	MOVLW       hi_addr(_ADDRESS_long_2+0
	MOVWF       FARG_set_long_address_address+1 
	CALL        _set_long_address+0, 0
;sensor.c,1109 :: 		set_short_address(ADDRESS_short_2);       // Set short address
	MOVLW       _ADDRESS_short_2+0
	MOVWF       FARG_set_short_address_address+0 
	MOVLW       hi_addr(_ADDRESS_short_2+0
	MOVWF       FARG_set_short_address_address+1 
	CALL        _set_short_address+0, 0
;sensor.c,1110 :: 		set_PAN_ID(PAN_ID_1);                     // Set PAN_ID
	MOVLW       _PAN_ID_1+0
	MOVWF       FARG_set_PAN_ID_address+0 
	MOVLW       hi_addr(_PAN_ID_1+0
	MOVWF       FARG_set_PAN_ID_address+1 
	CALL        _set_PAN_ID+0, 0
;sensor.c,1112 :: 		init_ZIGBEE_nonbeacon();                  // Initialize ZigBee module
	CALL        _init_ZIGBEE_nonbeacon+0, 0
;sensor.c,1113 :: 		nonbeacon_PAN_coordinator_device();
	CALL        _nonbeacon_PAN_coordinator_device+0, 0
;sensor.c,1114 :: 		set_TX_power(31);                         // Set max TX power
	MOVLW       31
	MOVWF       FARG_set_TX_power_power+0 
	CALL        _set_TX_power+0, 0
;sensor.c,1115 :: 		set_frame_format_filter(1);               // 1 all frames, 3 data frame only
	MOVLW       1
	MOVWF       FARG_set_frame_format_filter_fff_mode+0 
	CALL        _set_frame_format_filter+0, 0
;sensor.c,1116 :: 		set_reception_mode(1);                    // 1 normal mode
	MOVLW       1
	MOVWF       FARG_set_reception_mode_r_mode+0 
	CALL        _set_reception_mode+0, 0
;sensor.c,1118 :: 		pin_wake();                               // Wake from pin
	CALL        _pin_wake+0, 0
;sensor.c,1119 :: 		}
	RETURN      0
; end of _Initialize
...
_main:
	...
	MOVLW       255
	MOVWF       main_bestChannel_L0+0 
	CLRF        main_ch_cont_L0+0 
...
;sensor.c,1121 :: 		void main() {
;sensor.c,1129 :: 		Initialize();                      // Initialize MCU and Bee click board
	CALL        _Initialize+0, 0
;sensor.c,1131 :: 		while(1) {
L_main183:
;sensor.c,1132 :: 		if(sleep == 13){
	MOVLW       0
...
L__main236:
	BTFSS       STATUS+0, 2 
	GOTO        L_main185
;sensor.c,1133 :: 		PWR_reset();
	CALL        _PWR_reset+0, 0
;sensor.c,1134 :: 		write_ZIGBEE_short(SLPACK, 0x80);
	MOVLW       53
	MOVWF       FARG_write_ZIGBEE_short_address+0 
	MOVLW       128
	MOVWF       FARG_write_ZIGBEE_short_data_r+0 
	CALL        _write_ZIGBEE_short+0, 0
;sensor.c,1135 :: 		sleep = 0;
	CLRF        main_sleep_L0+0 
	CLRF        main_sleep_L0+1 
;sensor.c,1136 :: 		cond = 0;
	CLRF        main_cond_L0+0 
;sensor.c,1137 :: 		trans = 0;
	CLRF        main_trans_L0+0 
;sensor.c,1138 :: 		ch_cont++;
	INCF        main_ch_cont_L0+0, 1 
;sensor.c,1139 :: 		if(ch_cont == 200){
	MOVLW       0
	BTFSC       main_ch_cont_L0+0, 7 
	MOVLW       255
	MOVWF       R0 
	MOVLW       0
	XORWF       R0, 0 
	...
	GOTO        L__main237
	MOVLW       200
	XORWF       main_ch_cont_L0+0, 0 
L__main237:
	BTFSS       STATUS+0, 2 
	GOTO        L_main186
;sensor.c,1140 :: 		ch = bestChannel;
	MOVF        main_bestChannel_L0+0, 0 
	MOVWF       _ch+0 
;sensor.c,1141 :: 		bestChannel = 0xFF;
	MOVLW       255
	MOVWF       main_bestChannel_L0+0 
;sensor.c,1142 :: 		ch_cont = 0;
	CLRF        main_ch_cont_L0+0 
;sensor.c,1143 :: 		}
L_main186:
;sensor.c,1144 :: 		Lcd_Chr(1,5,'s');
	MOVLW       1
	MOVWF       FARG_Lcd_Chr_row+0 
	MOVLW       5
	MOVWF       FARG_Lcd_Chr_column+0 
...
L_main187:
	...
	DECFSZ      R11, 1, 0
	BRA         L_main187
;sensor.c,1147 :: 		Initialize();
	CALL        _Initialize+0, 0
;sensor.c,1148 :: 		}
L_main185:
;sensor.c,1152 :: 		if(trans == 0){
	MOVF        main_trans_L0+0, 0 
	XORLW       0
	BTFSS       STATUS+0, 2 
	GOTO        L_main188
;sensor.c,1153 :: 		Lcd_Chr(2,5,'b');
	MOVLW       2
	MOVWF       FARG_Lcd_Chr_row+0 
	MOVLW       5
	MOVWF       FARG_Lcd_Chr_column+0 
	...
	MOVWF       FARG_Lcd_Chr_out_char+0 
	CALL        _Lcd_Chr+0, 0
;sensor.c,1154 :: 		if(Debounce_INT() == 0 ){
	CALL        _Debounce_INT+0, 0
	MOVF        R0, 0 
	XORLW       0
	BTFSS       STATUS+0, 2 
	GOTO        L_main189
;sensor.c,1156 :: 		temp1 = read_ZIGBEE_short(INTSTAT); // Read and flush register INTSTAT
	MOVLW       49
	MOVWF       FARG_read_ZIGBEE_short_address+0 
	CALL        _read_ZIGBEE_short+0, 0
	MOVF        R0, 0 
	MOVWF       _temp1+0 
;sensor.c,1157 :: 		read_RX_FIFO();                     // Read receive data
	CALL        _read_RX_FIFO+0, 0
;sensor.c,1165 :: 		if(RSSI2 < bestChannel){
	MOVLW       128
	XORWF       _RSSI2+0, 0 
	MOVWF       R0 
	MOVLW       128
	XORWF       main_bestChannel_L0+0, 0 
	SUBWF       R0, 0 
	BTFSC       STATUS+0, 0 
	GOTO        L_main190
;sensor.c,1166 :: 		bestChannel = ch;
	MOVF        _ch+0, 0 
	MOVWF       main_bestChannel_L0+0 
;sensor.c,1167 :: 		}
L_main190:
;sensor.c,1168 :: 		lost = SN + lastSN;
	MOVF        main_lastSN_L0+0, 0 
	ADDWF       _SN+0, 0 
	MOVWF       R0 
;sensor.c,1169 :: 		lostT += lost;
	MOVF        main_lostT_L0+0, 0 
	ADDWF       R0, 1 
	MOVF        R0, 0 
	MOVWF       main_lostT_L0+0 
;sensor.c,1170 :: 		lastSN = SN;
	MOVF        _SN+0, 0 
	MOVWF       main_lastSN_L0+0 
;sensor.c,1171 :: 		EEPROM_Write(0x00, lostT);
	CLRF        FARG_EEPROM_Write_address+0 
	MOVF        R0, 0 
	MOVWF       FARG_EEPROM_Write_data_+0 
	CALL        _EEPROM_Write+0, 0
;sensor.c,1174 :: 		cond = 1;
	MOVLW       1
	MOVWF       main_cond_L0+0 
;sensor.c,1176 :: 		}
	GOTO        L_main191
L_main189:
;sensor.c,1177 :: 		else if(cond > 0){
	MOVLW       128
	MOVWF       R0 
	MOVLW       128
...
;sensor.c,1180 :: 		if(cond == 7){
	MOVF        main_cond_L0+0, 0 
	XORLW       7
	BTFSS       STATUS+0, 2 
	GOTO        L_main193
;sensor.c,1181 :: 		Initialize();                      // Initialize MCU and Bee click board
	CALL        _Initialize+0, 0
;sensor.c,1182 :: 		write_TX_normal_FIFO();
	CALL        _write_TX_normal_FIFO+0, 0
;sensor.c,1183 :: 		Lcd_Chr(1,1,'b');
	MOVLW       1
	MOVWF       FARG_Lcd_Chr_row+0 
	MOVLW       1
	MOVWF       FARG_Lcd_Chr_column+0 
...
L_main192:
L_main191:
;sensor.c,1187 :: 		} //final trans = 0
L_main188:
;sensor.c,1188 :: 		if(trans == 1){
	MOVF        main_trans_L0+0, 0 
	XORLW       1
	BTFSS       STATUS+0, 2 
	GOTO        L_main194
;sensor.c,1189 :: 		Delay_ms(9);
	MOVLW       30
	MOVWF       R12, 0
	MOVLW       55
	MOVWF       R13, 0
...
;sensor.c,1196 :: 		ch++;
	INCF        _ch+0, 1 
;sensor.c,1197 :: 		ch_cont = 0;
	CLRF        main_ch_cont_L0+0 
;sensor.c,1198 :: 		}
L_main197:
;sensor.c,1199 :: 		DATA_TX[0]='2';
	MOVLW       50
	MOVWF       _DATA_TX+0 
;sensor.c,1200 :: 		DATA_TX[1]='4';
	MOVLW       52
	MOVWF       _DATA_TX+1 
;sensor.c,1201 :: 		DATA_TX[2]=dig3;
	MOVF        _dig3+0, 0 
	MOVWF       _DATA_TX+2 
;sensor.c,1202 :: 		DATA_TX[3]=degrees;
	MOVF        _degrees+0, 0 
	MOVWF       _DATA_TX+3 
;sensor.c,1203 :: 		DATA_TX[4]=battery;
	MOVF        _battery+0, 0 
	MOVWF       _DATA_TX+4 
;sensor.c,1204 :: 		DATA_TX[15]=ch;
	MOVF        _ch+0, 0 
	MOVWF       _DATA_TX+15 
;sensor.c,1205 :: 		write_TX_normal_FIFO();
	CALL        _write_TX_normal_FIFO+0, 0
;sensor.c,1206 :: 		i = read_ZIGBEE_short(TXSTAT);
	MOVLW       36
	MOVWF       FARG_read_ZIGBEE_short_address+0 
	CALL        _read_ZIGBEE_short+0, 0
	MOVF        R0, 0 
	MOVWF       main_i_L0+0 
;sensor.c,1209 :: 		SEQ_NUMBER++;
	INCF        _SEQ_NUMBER+0, 1 
;sensor.c,1211 :: 		if((i & 1) == 0){
	MOVLW       1
	ANDWF       R0, 0 
	MOVWF       R1 
	MOVLW       0
...
L__main239:
	BTFSS       STATUS+0, 2 
	GOTO        L_main198
;sensor.c,1212 :: 		trans = 2;
	MOVLW       2
	MOVWF       main_trans_L0+0 
;sensor.c,1213 :: 		cond = 0;
	CLRF        main_cond_L0+0 
;sensor.c,1214 :: 		Lcd_Chr(1,1,'a');
	MOVLW       1
	MOVWF       FARG_Lcd_Chr_row+0 
	MOVLW       1
	MOVWF       FARG_Lcd_Chr_column+0 
...
L_main200:
L_main199:
;sensor.c,1219 :: 		}   //final trans ==1
L_main194:
;sensor.c,1221 :: 		sleep++;
	INFSNZ      main_sleep_L0+0, 1 
	INCF        main_sleep_L0+1, 1 
;sensor.c,1222 :: 		Delay_us(910);
	MOVLW       3
	MOVWF       R12, 0
	MOVLW       243
	MOVWF       R13, 0
