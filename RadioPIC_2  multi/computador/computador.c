...
short int temp1, ch = 11;   //ch canal que vai ligar
...
void main() {
     ...
     while(1) {
              //conta o tempo para fazer um novo pedido de dado
              if(cond3 == 62){
                       if(trans == 2){
              ...
              //verifica se preencheu toda EEPROM
              if(addr > 0xFF){
                      trans = 3;
              }
              //muda o canal automaticamente caso não consiga se comunicar (presumindo que houve dessincronização)
              if(ch_cond == 100){
                         if(ch == 26){
                            ch = 11;
                            ch_cond = 0;
